/**
 * Community Edition API Key Authentication Handler
 * 
 * Handles API key authentication for CE external integrations.
 * This is a simplified version that doesn't depend on EE packages.
 */
...
import { BaseSecurityHandler } from '../core/security/security-handler'
import { CeApiKey, ceApiKeyService } from './ce-api-key'

export class CeApiKeyAuthnHandler extends BaseSecurityHandler {
    ...
    protected canHandle(request: FastifyRequest): Promise<boolean> {
        const prefix = `${CeApiKeyAuthnHandler.HEADER_PREFIX}${CeApiKeyAuthnHandler.API_KEY_PREFIX}`
        const routeMatches = request.headers[CeApiKeyAuthnHandler.HEADER_NAME]?.startsWith(prefix) ?? false
    ...
    protected async doHandle(request: FastifyRequest): Promise<void> {
        ...
        let apiKey: CeApiKey | null = null
        try {
            apiKey = await ceApiKeyService.getByValueOrThrow(apiKeyValue)
        }
        catch (e) {
    ...
    private extractApiKeyValue(request: FastifyRequest): string {
        const header = request.headers[CeApiKeyAuthnHandler.HEADER_NAME]
        const prefix = CeApiKeyAuthnHandler.HEADER_PREFIX
